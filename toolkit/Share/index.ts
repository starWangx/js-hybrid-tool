...
export default {
  ...
  render(dom:any, config:any) {
    ...
    const getTmpl = (type:string) => {
      if (typesMap.indexOf(type) >= 0) {
        return `<div class="m-share-${type}"> </div>`;
      }
      return '';
    };
    let tmp = '';
  ...
  to(
    ...
    config: {
      ...
      diyShare: (type: 'wx' | 'wxline' | 'qq' | 'qzone' | 'sina' | 'copylink' , res?:boolean) => any; //不支持的浏览器定制分享的回调
    }
  ) {
    const _config = getDefaultConfig(config);
    init(_config);
    ...
    if(type === 'copylink'){
      return Copy(config.link||(typeof window !== 'undefined' && window.location.href || ''),res=>{
        if(res){
          config.diyShare && config.diyShare(type,res);
